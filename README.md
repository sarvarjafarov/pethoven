...
2. The workflow at `.github/workflows/deploy-hostinger.yml` syncs the project root (excluding uploads, scripts, etc.) to the remote `public_html/` folder on every push to `main` using rsync over SSH. Add your private key as `HOSTINGER_SSH_KEY` in the repo secrets (public key must be allowed in hPanel) and trigger the workflow manually or via pushes to `main`.
